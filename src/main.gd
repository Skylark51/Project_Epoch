...
func _build_game() -> Control:
    ...
    var map:=StrategicMap.new(); map.show_battlefield_units=true; maps[ScreenState.GAME]=map; map.province_selected.connect(_province_pick); map.selection_changed.connect(_selection_changed); map.province_dropped.connect(_quick_drag_move); map.command_target_selected.connect(_map_target); map.tooltip_changed.connect(_map_tooltip); map.city_selected.connect(_on_map_city_selected); map_panel.add_child(map)
